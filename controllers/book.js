...
export const deleteBook = async (req, res) => {
    try {
        ...
        let { id } = req.params;
        let book = await Book.findById(id);
        if (book.userAdded != req.myUser._id)
            return res.status(403).send("you cannot delete books you didnt add")
...
export const updateBook = async (req, res) => {
    try {
        ...
        if (!mongoose.isValidObjectId(id))
            ...
            let book = await Book.findById(id);
            if (book.userAdded != req.myUser._id)
                return res.status(403).send("you cannot update books you didnt add")
...
export const addBook = async (req, res) => {
    ...
    try {
        ...
        let newBook = new Book({ name, numP, author:"none" ,userAdded:req.myUser._id});
        await newBook.save();
